...
class TestProfileService(TestCase):
    ...
    @mock.patch(
        "requests.get", autospec=True, spec_set=True, return_value=MockResponse(200, fixtures.invalid_user_data())
    )
    ...
    @mock.patch(
        "requests.get", autospec=True, spec_set=True, return_value=MockResponse(200, fixtures.profile_raw_data())
    )
...
class TestAchievementService(TestCase):
    @mock.patch(
        ...
        autospec=True,
        spec_set=True,
        return_value=MockResponse(200, fixtures.invalid_player_stats_data()),
    )
