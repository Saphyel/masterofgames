...
class TestProfileService:
    @patch("masterofgames.crawlers.user.client_fetch", return_value=fixtures.user_data(), new_callable=AsyncMock)
    async def test_valid_user_data(self, mock_requests):
        assert "666" == await ProfileService().get_user_id("Saphyel")

    @patch("masterofgames.crawlers.user.client_fetch", new_callable=AsyncMock, return_value=fixtures.profile_raw_data())
    @patch("masterofgames.crawlers.player.client_fetch", new_callable=AsyncMock, return_value=fixtures.profile_raw_data())
...
class TestAchievementService:
    @patch("masterofgames.crawlers.stats.client_fetch", new_callable=AsyncMock, return_value=fixtures.achievement_raw_data())
